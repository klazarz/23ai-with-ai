...
def mongo_connect():
    client = MongoClient(
        "mongodb://ora23ai:ora23ai@ords:27017/ora23ai?authMechanism=PLAIN&authSource=$external&ssl=true&retryWrites=false&loadBalanced=true&tlsAllowInvalidCertificates=true"
    )
    db = client["ora23ai"]
    return db
...
def get_employees():
    cursor.execute(
        """SELECT ID, PRODUCT_ID, ORDER_DATE, CUSTOMER_ID, TOTAL_VALUE, ORDER_SHIPPED FROM NEW_ORDERS
                """
    )
    title = [i[0] for i in cursor.description]
    data = cursor.fetchall()
...
def get_dv():
    cursor.execute(
        """SELECT * FROM CUSTOMERS_DV c where json_exists(c.data, '$?(@.FirstName == "Dom")')"""
    )
    data = cursor.fetchall()
...
def get_mongo():
    ...
    mongo_data = col.find_one({"FirstName": "Dom"})
...
def update_mongo(new):
    col = mongo_connect().CUSTOMERS_DV
    col.update_one({"FirstName": "Dom"}, {"$set": {"Email": new}})
    return
...
def put_rest(customer_id, new_email):
    ...
    url = f"http://ords:8181/ords/ora23ai/customers_dv/{customer_id}"
...
def get_vector_search_ollama():
    ...
    # Replace 'cat' in the query with the user-provided word
    query = f"""SELECT DBMS_VECTOR_CHAIN.UTL_TO_EMBEDDING('{word}', JSON('{{"provider":"ollama", "host": "local","url": "http://ollama:11434/api/embeddings", "model":"deepseek-r1:1.5b"}}') )"""
    cursor.execute(query)
    title = [i[0] for i in cursor.description]
    data = cursor.fetchall()
...
def get_simsearch_ollama():
    ...
    if request.method == "POST":
        ...
        # Construct and execute query
        query = f"""SELECT VECTOR_DISTANCE(
                            DBMS_VECTOR_CHAIN.UTL_TO_EMBEDDING('{word1}', JSON('{{"provider":"ollama", "host": "local","url": "http://ollama:11434/api/embeddings",  "model":"deepseek-r1:1.5b"}}')), 
                            DBMS_VECTOR_CHAIN.UTL_TO_EMBEDDING('{word2}', JSON('{{"provider":"ollama", "host": "local","url": "http://ollama:11434/api/embeddings",  "model":"deepseek-r1:1.5b"}}')),
                            COSINE )"""
        cursor.execute(query)
        data = cursor.fetchall()
        distance = data[0][0] if data else "N/A"  # Assuming single result for distance
