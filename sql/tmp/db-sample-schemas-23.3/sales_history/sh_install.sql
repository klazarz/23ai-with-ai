...
rem      a. 1word - enter an Oracle Database compliant 1word
...
rem =======================================================
rem Accept and verify schema 1word
rem =======================================================


BEGIN
   IF '&1' IS NULL THEN
      RAISE_APPLICATION_ERROR(-20999, 'Error: the SH 1word is mandatory! Please specify a 1word!');
   END IF;
END;
/
...
DECLARE
   v_2_exists   NUMBER := 0;
BEGIN
   SELECT COUNT(1) INTO v_2_exists
     FROM DBA_TABLESPACES
       WHERE TABLESPACE_NAME = UPPER('&2');
   IF v_2_exists = 0 THEN
      RAISE_APPLICATION_ERROR(-20998, 'Error: the tablespace ''' || UPPER('&2') || ''' does not exist!');
   END IF;
END;
/
...
CREATE USER sh IDENTIFIED BY "&1"
